...
import pyodbc
import pandas as pd
import packages.remove.bank.filegen_sbki as filegen_sbki
import packages.remove.bank.filegen_nbki as filegen_nbki
import warnings
...
def header(conn,number):
    ...
    df.to_csv('input/bankruptcy.csv', index = False, mode = 'w', sep='*', header= True)
...
def lime(conn,number,dfExists,dfEmpty,dfTrailer):
	...
	if df.empty:
		...
		df.to_csv('input/bankruptcy.csv', index = False, mode = 'a', sep='*', header= False)
...
def main(conumbers,project,bki_type):
